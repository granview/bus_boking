...
async function executeSecureAction() {
    ...
    try {
        ...
        // =========================
            ...
            if (isFutureDate(dragBooking.date)) {
                updates[dragBooking.id] = null;
            }
            else {
...
async function exportExcel() {
    try {
        ...
        const earlySheet = workbook.getWorksheet("6-7");
        const lateSheet = workbook.getWorksheet("7-10");
        ...
        earlySheet.getCell("D1").value = headerText;


        lateSheet.getCell("D1").value = headerText;
        ...
        Object.values(earlyCellMap).forEach(addr => {

            earlySheet.getCell(addr).value = "";

        });

        Object.values(lateCellMap).forEach(addr => {

            lateSheet.getCell(addr).value = "";
        ...
        Object.entries(grouped).forEach(([key, list]) => {
            ...
            if (earlyCellMap[key]) {
                ...
                sheet = earlySheet;
                baseCell = earlyCellMap[key];

            }

            // LATE
            else if (lateCellMap[key]) {

                sheet = lateSheet;
                baseCell = lateCellMap[key];
            ...
            list.forEach((text, i) => {
                ...
                const rowHeight = Math.max(20, text.length / 2);
