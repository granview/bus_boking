...
async function loadReservations() {
   ...
   const selectedDate = adminDate.value;
    ...
    // === HOÁN ĐỔI TÊN CỘT HEADER KHI STOP ===
    const thCells = document.querySelectorAll('#pageEarlyView .th-cell.car-col');
    if (thCells.length >= 2) {
        if (hotelEarlyStop) {
            thCells[0].innerHTML = 'ホテル車';
            thCells[1].innerHTML = 'ラッキータクシー <button id="hotelStopBtn" class="stop-btn">STOP</button>';
        } else {
            thCells[0].innerHTML = 'ラッキータクシー';
            thCells[1].innerHTML = 'ホテル車 <button id="hotelStopBtn" class="stop-btn">STOP</button>';
        }
    }
                ...
                bookingList.forEach(item => {
                    ...
                    if (!isCanceled && !isMoved) {
                        ...
                        const getDropBoxAt = (clientX, clientY) => {
                            // Ẩn ghost tạm để elementFromPoint xuyên qua được
                        ...
                        const handlePointerMove = (e) => {
                            ...
                            if (!pointerDragState.dragging) {
                                ...
                                ghostEl.style.cssText = `
                                    ...
                                    left: ${e.clientX - pointerDragState.offsetX}px;
                                    top: ${e.clientY - pointerDragState.offsetY}px;
                                `;
                            ...
                            if (pageButton) {
                                const now = Date.now();
                                if (now - lastPageSwitchAt > 500) {
                                    lastPageSwitchAt = now;
                                    pageButton.click();
                                }
                            }

                            // 2. Highlight ô đang rê chuột qua
...
const isStopped = stopSnap.exists(); // Kết quả là true nếu ngày đó có "STOP"
        ...
        // Khi STOP: hoán đổi cột C (ラッキータクシー) ↔ D (ホテル車)
        const earlyMap = isStopped ? {
            "06:05_ラッキータクシー": "D5",  "06:05_ホテル": "C5",
            "06:20_ラッキータクシー": "D15", "06:20_ホテル": "C15",
            "06:40_ラッキータクシー": "D25", "06:40_ホテル": "C25",
            "07:00_ラッキータクシー": "D35", "07:00_ホテル": "C35"
        } : {
        ...
        Object.values(data).forEach(item => {
            ...
            // Hành lý bọc trong ()
            const lugParts = [];
            if (item.tokudai > 0) lugParts.push(`特大${item.tokudai}`);
            if (item.large > 0) lugParts.push(`大${item.large}`);
            if (item.medium > 0) lugParts.push(`中${item.medium}`);
            if (item.small > 0) lugParts.push(`小${item.small}`);
            if (item.note && item.note.trim()) lugParts.push(item.note.trim());
            const luggage = lugParts.length > 0 ? `(${lugParts.join(" ")})` : "";
            ...
            // Ghép phần đầu: 部屋｜名前様｜人数
            const mainPart = `${item.room || ""}｜${item.name || ""}様｜${paxText}${iconText}${stay}`;
            // Nếu có hành lý thì xuống dòng
            const text = luggage ? `${mainPart}
${luggage}` : mainPart;
        ...
        // Hoán đổi header C3/D3 khi STOP
        if (sheetEarly) {
            if (isStopped) {
                sheetEarly.getCell("C3").value = "ホテル";
                sheetEarly.getCell("D3").value = "ラッキータクシー";
            } else {
                sheetEarly.getCell("C3").value = "ラッキータクシー";
                sheetEarly.getCell("D3").value = "ホテル";
            }

            // Đảm bảo 1 trang khi in
            sheetEarly.pageSetup.fitToPage = true;
            sheetEarly.pageSetup.fitToWidth = 1;
            sheetEarly.pageSetup.fitToHeight = 1;
            sheetEarly.pageSetup.paperSize = 9;
            sheetEarly.pageSetup.orientation = "portrait";
        }
