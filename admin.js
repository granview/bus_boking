...
if (!isCanceled && !isMoved) {
    ...
    line.addEventListener("touchmove", (e) => {
        ...
        const dropBox = target?.closest(".car-cell-box");
        ...
        if (dropBox) {
            ...
            // AUTO TAB SWITCH
            const time = dropBox.dataset.time;
            ...
            else if (lateHours.includes(time)) {
    ...
    line.addEventListener("touchend", (e) => {
        ...
        if (!touchDragging) {
            ...
            stayToggle.textContent = stayActive
                ? "ON"
                : "OFF";
        ...
        // same slot
        if (
            dragBooking.time === pendingTargetTime &&
            dragBooking.car === pendingTargetCar
        ) {
